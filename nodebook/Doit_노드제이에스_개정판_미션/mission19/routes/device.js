		...
		database.DeviceModel.findAll(function(err, results) {
			...
			if (results) {
				...
				for (var i = 0; i < results.length; i++) {
                    ...
                    var curMobile = results[i]._doc.mobile;
					console.log('등록 ID #' + i + ' : ' + regIds.length);
                    console.log('전화번호 #' + i + ' : ' + curMobile);
					
					regIds.push(curId);
                    mobiles.push(curMobile);
				}
				...
				sender.send(message, regIds, function (err, result) {
                    ...
                    // 푸시 이력 저장
                    addHistory(database, mobiles, paramData);
...
/*
 * history 스키마에 전송 기록 추가
 *
 * mobiles 전화번호 배열
 * data 전송 내용
 */
var addHistory = function(database, mobiles, data) {
    
	// 데이터베이스 객체가 초기화된 경우
	if (database.db) {
		
		// HistoryModel 인스턴스 생성
        for (var i = 0; i < mobiles.length; i++) {
            var history = new database.HistoryModel({"mobile":mobiles[i], "contents":data});

            // save()로 저장
            history.save(function(err) {
                if (err) {
                    console.error('푸시 이력 추가 중 에러 발생 : ' + err.stack);
                    return;
                }

                console.log("푸시 이력 추가함 : " + mobiles[i]);
            });
        }  
