...
var findCircle = function(req, res) {
	...
	if (database.db) {
		// 1. 가까운 커피숍 검색
		database.ShopModel.findCircle(paramCenterLongitude, paramCenterLatitude, paramRadius, function(err, results) {
			if (err) {
			...
			if (results) {
				...
				if (results.length > 0) {
					res.render('circleshop.ejs', {result: results[0]._doc, paramLatitude: paramCenterLatitude, paramLongitude: paramCenterLongitude});
				} else {
					res.writeHead('200', {'Content-Type':'text/html;charset=utf8'});
